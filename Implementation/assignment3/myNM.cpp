...
Description     : Assignment 3 - myNM.cpp
/------------------------------------------------------------------------------------------*/
...
void partialPivoting(Matrix _U, Matrix _b, Matrix _P, int _k) 
{
	...
	if ((fabs(_U.at[_k][_k]) / max) < 0.001)		// > change
		return;
...
{
	double m = 0;
	_P = eye(_P.rows, _P.cols);
	_U = copyMat(_A);

	for (int i = 0; i <_A.rows ; i++)
	{
		partialPivoting(_A, _bn, _P, i);
		
		for (int j = i + 1; j <_A.rows ; j++)
		{
...
void LUdecomp(Matrix _A, Matrix &_L, Matrix &_U, Matrix _b, Matrix _P)
{
	...
	int max_idx;
	double m = 0, temp = 0, max = 0;
	_U = copyMat(_A);

	for (int k = 0; k < (_L.rows - 1); k++)
	{
		partialPivoting(_U, _b, _P, k);
		printMat(_U);
		// elimination
		for (int i = k + 1; i < _U.rows; i++)
		{
			m = (_U.at[i][k] / _U.at[k][k]);		// make m matrix for the efficient calculation
			if (_U.at[k][k] == 0)								// error case: divide by zero
			{
				printf("A[%d][%d] is zero that divides by zero!!\n", k, k);
				break;
			}
			else if (_U.rows != _U.cols)					// error case: not square matrix
			{
			...
			_L.at[i][k] = m;
			for (int j = k; j < _U.cols; j++)					// gauss-elimination
			{
				_U.at[i][j] = _U.at[i][j] - m * _U.at[k][j];
			}
		}
	}
