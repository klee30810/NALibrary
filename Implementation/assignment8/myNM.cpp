...
{
	// Function from the given formula
	return 0.565 * _x*_x + 4.17 * _x -0.9;
}
...
double LagrangeTNT(Matrix _x, Matrix _y, double _xint)
{
	int n = _x.rows;
	double _yint = 0;
	Matrix L = zeros(n, 1);
	...
	for (int i = 0; i < n; i++)
	{
		...
		{
			if (j != i)
				L.at[i][0] = L.at[i][0] * (_xint - _x.at[j][0]) / (_x.at[i][0] - _x.at[j][0]);
		}
	}

	for (int k = 0; k < n; k++)
	{
		_yint += (_y.at[k][0] * L.at[k][0]);
	}
...
double NewtonsTNT(Matrix _x, Matrix _y, double _xint)
{
	...
	for (int i = 0; i < (n - 1); i++)
	{
		divDIF.at[i][0] = (_y.at[i + 1][0] - _y.at[i][0]) / (_x.at[i + 1][0] - _x.at[i][0]);
	}
	...
	for (int j = 2; j <= (n - 1); j++)
	{
		for (int i = 0; i < (n - j); i++)
		{
			divDIF.at[i][j-1] = (divDIF.at[i + 1][j - 2] - divDIF.at[i][j - 2]) / (_x.at[j + i][0] - _x.at[i][0]);
		}
	}

	for (int i = 1; i < n; i++)
	{
		a.at[i][0] = divDIF.at[0][i-1];
	}
	
	_yint = a.at[0][0];
	for (int k = 1; k < n; k++)
	{
		xn *= _xint - _x.at[k - 1][0];
		_yint += a.at[k][0] * xn;
	}
...
double examplefunc(double _x, double _y)
{
	...
	result = 3 * exp(-_x) - 0.4 * _y;

	return result;
}

/* Solving one 1st order ODE with 2nd order Runge-Kutta (Heun Method)
	...
	_X: output matrix x
	_Y output matrix y:*/
void odeRK2(double _func(double _x, double _y), double _a, double _b, double _h, double _yinit, Matrix _X, Matrix _Y)
{
	double k1 = 0, k2 = 0;
	double N = (_b - _a) / _h;
	
	_X.at[0][0] = _a; _Y.at[0][0] = _yinit;

	for (int i = 0; i < N; i++)
	{
		k1 = _func(_X.at[i][0], _Y.at[i][0]);
		k2 = _func(_X.at[i][0] + _h, _Y.at[i][0] + k1 * _h);
		_Y.at[i + 1][0] = _Y.at[i][0] * (1 / 2 * k1 + 1 / 2 * k2)*_h;
		_X.at[i + 1][0] = _X.at[i][0] * _h;
	}
	
}

/* Solving one 1st order ODE with 3rd order Runge-Kutta (classical method)
	_func: input 1st derivative function
	_a: left limit of the range
	_b: right limit of the range
	_h: size of interval
...
void odeRK3(double _func(double _x, double _y), double _a, double _b, double _h, double _yinit, Matrix _X, Matrix _Y)
{
	...
	{
		k1 = _func(_X.at[i][0], _Y.at[i][0]);
		k2 = _func(_X.at[i][0] + _h / 2, _Y.at[i][0] + k1 * _h / 2);
		k3 = _func(_X.at[i][0] + _h, _Y.at[i][0] - k1 * _h + 2 * k2 * _h);
		_Y.at[i + 1][0] = _Y.at[i][0] * (k1 + 4 * k2 + k3)*_h / 6;
		_X.at[i + 1][0] = _X.at[i][0] * _h;
	}
...
{
	double m = 1, k = 3, c = 20, A = 2, f = 5;
	double dvdt = 0;

	dvdt = -c / m * _v - k / m * _y + A / m * cos(2 * 3.141592*f*_x);

	return dvdt;
}


/* Solving mck ODEs with 3rd order Runge-Kutta (classical method)
...
{
	_X.at[0][0] = _a; _Y.at[0][0] = _yinit; _V.at[0][0] = _vinit;
	double N = (_b - _a) / _h;
	double Kx1 = 0, Ky1 = 0, Kx2 = 0, Ky2 = 0, Kx3 = 0, Ky3 = 0;

	for (int i = 0; i < (N-2); i++)
	{
		Kx1 = _mckdydt(_X.at[i][0], _Y.at[i][0], _V.at[i][0]);
		Ky1 = _mckdvdt(_X.at[i][0], _Y.at[i][0], _V.at[i][0]);

		Kx2 = _mckdydt(_X.at[i][0], _Y.at[i][0] + _h / 2, _V.at[i][0] + _h / 2 * Kx1);
		Ky2 = _mckdvdt(_X.at[i][0], _Y.at[i][0] + _h / 2, _V.at[i][0] + _h / 2 * Ky1);

		Kx3 = _mckdydt(_X.at[i][0], _Y.at[i][0] + _h, _V.at[i][0] - Kx1 * _h + 2 * Kx2 * _h);
		Ky3 = _mckdvdt(_X.at[i][0], _Y.at[i][0] + _h, _V.at[i][0] - Ky1 * _h + 2 * Ky2 * _h);

		_Y.at[i + 1][0] = _Y.at[i][0] + (Kx1 + 4 * Kx2 + Kx3)*_h / 6;
		_V.at[i + 1][0] = _V.at[i][0] + (Ky1 + 4 * Ky2 + Ky3)*_h / 6;
		_X.at[i + 1][0] = _X.at[i][0] + _h;
	}
